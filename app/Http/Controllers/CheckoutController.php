...
class CheckoutController extends Controller
{
    ...
    public function renderCheckoutShipping()
    {
        ...
        return view('front.'. $this->device .'.dynamic.order-shipping', compact('carts'));
    }
    ...
    public function renderThankyouPage(Request $request)
    {
        ...
        return view('front.'. $this->device .'.dynamic.thanks', compact('promocode', 'order'));
    }
    ...
    {
        // dd($methodId, $amount, $orderId, $payment);
     ...
     */
     public function countAmount($order)
     {
         ...
         foreach ($order->orderSubproducts as $key => $subproductItem) {
             $amount['main']['amount'] += $subproductItem->subproduct->product->mainPrice->price;
             $amount['personal']['amount'] += $subproductItem->subproduct->product->personalPrice->price;
         }
    ...
    public function changeCountry(Request $request)
    {
        ...
        setcookie('country_id', $data['country']->id, time() + 10000000, '/');
        setcookie('delivery_id', $data['country']->mainDelivery->delivery_id, time() + 10000000, '/');
