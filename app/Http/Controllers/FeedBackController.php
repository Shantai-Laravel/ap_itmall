...
class FeedBackController extends Controller
{
    ...
    public function contactFeedBack(Request $request)
    {
        ...
        $email1 = "sales@anapopova.com";
        $email2 = "showroom@anapopova.com";
        $email3 = "byanapopova@gmail.com";
        ...
        Mail::send('mails.contactForm.admin', $data, function($message) use ($email1){
            $message->to($to, 'Contactează-ne')->from('info@annepopova.com')->subject('Contact Us.');
        });

        Mail::send('mails.contactForm.admin', $data, function($message) use ($email2){
            $message->to($to, 'Contactează-ne')->from('info@annepopova.com')->subject('Contact Us.');
        });

        Mail::send('mails.contactForm.admin', $data, function($message) use ($email3){
            $message->to($to, 'Contactează-ne')->from('info@annepopova.com')->subject('Contact Us.');
        });
        ...
        Mail::send('mails.contactForm.user', $data, function($message) use ($email){
            $message->to($email, 'You left a message on the online shop annepopova.com')->from('info@annepopova.com')->subject('You left a message on the online shop annepopova.com');
        });

        Session::flash('contacts-success', 'Va multumim, in scrut timp managerii nostri va vor contacta.');
        return redirect()->back();
    }
}
