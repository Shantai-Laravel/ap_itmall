...
class CartController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('front.'. $this->device .'.dynamic.cart', compact('seoData'));
    }
    ...
    public function addProductToCart(Request $request)
    {
        ...
        if ($product->subproducts->count() > 0) {
            ...
            $cart = Cart::where('user_id', $this->userId)->where('subproduct_id', 0)->where('product_id', $product->id)->first();
    ...
    public function diableSetDiscount(Request $request)
    {
        ...
        $setCarts = Cart::where('user_id', $this->userId)->get();
     ...
     */
     ...
     public function getCountries()
     {
         ...
         if (is_null($currentCountry) || is_null($currentCountry->mainDelivery)){
             ...
             if (!$userDelivery) {
                 $data['mainDelivery'] = $data['currentCountry']->mainDelivery->delivery_id;
             }else{
                 $data['mainDelivery'] = $userDelivery;
             }
         }else{
             $data['currentCountry'] = $currentCountry;
             if (!$userDelivery) {
                 $data['mainDelivery'] = $currentCountry->mainDelivery->delivery_id;
             }else{
                 $data['mainDelivery'] = $userDelivery;
             }
         }
    ...
    public function validateStocks($userId)
    {
     ...
     public function exchangeShippingPrice(Request $request)
     {
         ...
         if ($request->get('shippingId')) {
             // if (!@$_COOKIE['delivery_id']) {
                 setcookie('delivery_id', $request->get('shippingId'), time() + 10000000, '/');
             // }
         }
