...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        $data['name'] = $user->name;
        $subject = trans('vars.Email-templates.emailRegistrationSubject').'info@annepopova.com';
    ...
    */
    ...
    public function changePassword(Request $request)
    {
        ...
        if(!is_null($user)){
            ...
            $user->pass_str = $request->get('password');
    ...
    public function registrationStatic(Request $request)
    {
        $validator = validator($request->all(), [
            'email' => 'required|unique:front_users',
            'name' => 'required|min:3',
        ...
        $email = $request->get('email');
        $email1 = 'byanapopova@gmail.com';
        $email2 = 'sales@anapopova.com';
        $email3 = 'itmalles@gmail.com';

        $status = Mail::send('mails.auth.register', $data, function($message) use ($email1) {
            $message->to($email1);
            $message->from('info@annepopova.com');
            $message->subject('new B2B Client'.'info@annepopova.com');
        });

        $status = Mail::send('mails.auth.register', $data, function($message) use ($email2) {
            $message->to($email2);
            $message->from('info@annepopova.com');
            $message->subject('new B2B Client'.'info@annepopova.com');
        });

        $status = Mail::send('mails.auth.register', $data, function($message) use ($email3) {
            $message->to($email3);
            $message->from('info@annepopova.com');
            $message->subject('new B2B Client'.'info@annepopova.com');
        });
        ...
        $status = Mail::send('mails.auth.register', $data, function($message) use ($email) {
            $message->to($email);
            $message->from('info@annepopova.com');
            $message->subject(trans('vars.Email-templates.emailRegistrationSubject').'info@annepopova.com');
        });

        Session::flash('success', 'success');
        return redirect('/ro/bijoux');
    }
    ...
    */
    public function loginStatic(Request $request)
    {
        $validator = validator($request->all(), [
            'email' => 'required',
            'password' => 'required'
        ...
        $user = FrontUser::where('pass_str', $request->get('password'))
        ...
        if (!is_null($user)){
            ...
            return redirect()->back()->withErrors(['email' => 'auth faled!']);
        }
