...
class PaymentHandler extends Controller
{
    ...
    public function handle()
    {
        if (self::$payment == 'cash'){              // done
            ...
            $paynet = new Paynet();
            return $paynet->index(self::$order);
        }
    }
    ...
    public function success()
    {
        ...
        $emailUser =  self::$order->details->email;
        $email_1 = "sales@anapopova.com";
        $email_2 = "showroom@anapopova.com";
        $email_3 = "byanapopova@gmail.com";
        $email_4 = "itmalles@gmail.com";
        ...
        $mail->sendEmailAttach($data, $email_1, $subject, $template, $path, $filename);
        $mail->sendEmailAttach($data, $email_2, $subject, $template, $path, $filename);
        $mail->sendEmailAttach($data, $email_3, $subject, $template, $path, $filename);
        $mail->sendEmailAttach($data, $email_4, $subject, $template, $path, $filename);
    ...
    public function generateInvoice()
    {
        ...
        $data['promocode']  = '';
        $data['currencyRate'] = $this->currency->rate;
        $invoiceName = "invoices.invoice_en";

        try {
            if (auth('persons')->user()->active_diller == 1) {
                $invoiceName = "invoices.invoice_diller";
            }
        } catch (\Exception $e) {}


        $pdf        = PDF::loadView($invoiceName, $data)->setPaper('a4', 'portrait');
        $path       = public_path('pdf/');
