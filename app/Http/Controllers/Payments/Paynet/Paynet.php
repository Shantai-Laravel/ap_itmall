...
use App\Http\Controllers\Payments\Paynet\PaynetRequest;
use App\Http\Controllers\Payments\Paynet\PaynetResult;
use App\Http\Controllers\Payments\Paynet\PaynetCode;
use App\Http\Controllers\Payments\Paynet\PaynetEcomAPI;
use App\Models\CRMOrders;


class Paynet extends PaymentHandler
{
	public function index($order)
	{
		...
		$prequest->LinkSuccess 	= url('/paynet-success-link/'.$order->id.'/'.self::$payment)."?id=$prequest->ExternalID";
		$prequest->LinkCancel 	= url('/paynet-cancel-link/'.$order->id.'/'.self::$payment)."?id=$prequest->ExternalID";
		$prequest->Lang = 'ru';
		...
		if ($order->orderSubproducts()->count() > 0) {
            foreach ($order->orderSubproducts as $key => $subproductItem) {
                $orderProducts[] = [
				   'LineNo' 	=> $key + 1,
				   'Code' 		=> $subproductItem->subproduct->product->code,
				   'Barcode' 	=> $subproductItem->subproduct->product->code,
        ...
        if ($order->orderProducts()->count() > 0) {
            foreach ($order->orderProducts as $key => $productItem) {
                $orderProducts[] = [
				   ...
				   'Barcode' => $productItem->product->code,
	...
	public function successLink($orderId, $payment)
	{
		self::$order = CRMOrders::findOrFail($orderId);
	...
	{
		self::$order = CRMOrders::findOrFail($id);
        self::$payment = $payment;
