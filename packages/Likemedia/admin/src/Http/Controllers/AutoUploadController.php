...
use App\Models\AdjacentProduct;
...
class AutoUploadController extends Controller
{
    ...
    {
        $images = ProductImage::where('product_id', $request->get('product_id'))->where('type', null)->orWhere('type', 'set')->orderBy('id', 'desc')->get();
        return $images;
    }
    ...
    public function removeImage(Request $request)
    {
        ...
        $images = ProductImage::where('product_id', $request->get('product_id'))->where('type', null)->where('type', 'set')->orderBy('id', 'desc')->get();
        return $images;
    }
    ...
    public function mainImage(Request $request)
    {
        ...
        // $array = [null, 'set'];
        //
        // $images = ProductImage::where('product_id', $request->get('product_id'))
        //                     ->whereIn('type', $array)
        //                     // ->orWhere('type', 'set')
        //                     ->orderBy('id', 'desc')
    ...
    public function setImage(Request $request)
    {
        ...
        if (!is_null($image)) {
            ProductImage::where('product_id',$request->get('product_id'))->where('type', 'set')->update([
                'type' => null,
            ]);

            ProductImage::where('id', $request->get('id'))->update([
                'type' => 'set',
            ]);
        }

        $images = Product::where('id', $request->get('product_id'))->first();
    ...
    public function firstImage(Request $request)
    {
        ...
        $data['images'] = ProductImage::where('product_id', $request->get('product_id'))->where('deleted', null)->where('type', null)->where('type', 'set')->orderBy('id', 'desc')->orderBy('main', 'asc')->get();
    ...
    public function getAdjacentProductList(Request $request)
    {
        ...
        $data['adjacents']  = Product::with($this->productRelationship)
                                    ->whereIn('id', $someColorProdIds)
                                    ->where('id', '!=', $product->id)
                                    ->where('active', 1)
                                    ->get();

        $data['similars']  = Product::with($this->productRelationship)
    ...
    public function selectAdjacentProductList(Request $request)
    {
        $chekProduct = AdjacentProduct::where('product_id', $request->get('productId'))
                                        ->where('adjacent_id', $request->get('adjacentId'))
                                        ->first();
        if (is_null($chekProduct)) {
            AdjacentProduct::create([
                'product_id' => $request->get('productId'),
                'adjacent_id' => $request->get('adjacentId'),
            ]);
        }else{
            $chekProduct->delete();
        }
    ...
    public function selectSimilarProductList(Request $request)
    {
        $chekProduct = Similar::where('product_id', $request->get('productId'))
                                        ->where('similar_id', $request->get('similarId'))
                                        ->first();
        if (is_null($chekProduct)) {
            Similar::create([
                'product_id' => $request->get('productId'),
                'similar_id' => $request->get('similarId'),
            ]);
        }else{
            $chekProduct->delete();
        }
