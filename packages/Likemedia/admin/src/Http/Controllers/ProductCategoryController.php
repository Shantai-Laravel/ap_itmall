...
class ProductCategoryController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $loungewear = 0;
        $jewelry    = 0;
        $icon_desktop = $request->get('old_icon') ?? null;
        ...
        if($file = $request->file('icon')){
            $icon_desktop = uniqid(). '-' .$file->getClientOriginalName();
            $image_resize = Image::make($file->getRealPath());

            $image_resize->save(public_path('images/categories/og/' .$icon_desktop), 75);

            $image_resize->resize(480, null, function ($constraint)
                                            {
                                                $constraint->aspectRatio();
                                            })->save('images/categories/sm/' .$icon_desktop);
        ...
        $productCategory->icon = $icon_desktop;
    ...
    public function deleteBanner($device, $id)
    {
        ...
        if($device == 'icon'){
            ...
            @unlink(public_path('/images/categories/og/' . $collection->icon));
        }
