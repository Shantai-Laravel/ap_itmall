...
>
    ...
    <body>
        @php
            ...
            foreach ($order->orderProducts as $key => $product){
                if (!is_null($product->product)){
                    $amount +=  $product->product->personalPrice->price * $product->qty;
        ...
        <div class="email-container">
            <table class="section">
                <td class="logo">
                    <img src="https://annepopova.com/fronts/img/icons/logo.png" alt="">
                </td>
            ...
            <div class="section table-middle">
              <table>
                <td class="furnizor">
                    ...
                    <div class="factura">Homewear: AnaGeo Textil SRL</div>
                    <p>Tax-ID: <b>1017600016064</b></p>
                    <p>IBAN: MD97AG000000022513122365</p>
                    <div class="factura">Jewelry: Ju Bijoux SRL</div>
                    <p>Tax-ID: <b>1017600037432</b></p>
                    <p>IBAN: MD81AG000000022513224697</p>
                    <p>BANK: B.C. “MOLDOVA-AGROINDBANK” S.A.br. NR. 15 CHISINAU, SWIFT: AGRNMD2X</p>
                    <p>CORRESPONDENT BANK: COMMERZBANK AG., FRANKFURT AM MAIN, GERMANY , SWIFT: COBADEFF</p>
                    <p>Address: Moldova, Chisinau, str. Puskin A., 5B</p>
                    <p>Showroom: Moldova, Chisinau, str. Puskin 8</p>
                    <p>Email: sales@annepopova.com</p>
                    <p>Tel: (+373) 79188090, (+373) 79488188</p>
                    <p>Shipping method: {{ $order->details->delivery }}</p>
                    <p>Payment method: {{ $order->payment_id }}</p>
                </td>
                <td class="furnizor">
                    <div class="title">
                        Client
                    ...
                    <p>Tel: +{{ $order->details->code }} {{ $order->details->phone }}</p>
                    <p>Address: {{ $order->details->country }}, {{ $order->details->city }}, {{ $order->details->address }}</p>
                    <p>Email: {{ $order->details->email }}</p>
                </td>
              </table>
            </div>
            <div class="section table-list"> <br><br><br><br><br><br><br><br><br><br><br><br>
                <table>
                    <tr class="tableHead">
                        <th>Nr. crt</th>
                        <th>Products</th>
                        {{-- <th>U.M.</th> --}}
                        <th>Qty</th>
                        <th>Unit Price {{ $currency->abbr }}</th>
                        <th>Amount {{ $currency->abbr }}</th>
                    ...
                    @foreach ($order->orderProducts as $key => $product)
                        @if (!is_null($product->product))
                            <tr>
                                ...
                                <td>{{ number_format((float)$product->product->personalPrice->price * $product->qty, 2, '.', '')  }}</td>
