    ...
    <div style="position: relative;" v-if="cartProds.length || cartSubprods.length">
        <div class="orderSummary">
            <div>
                    ...
                    <div class="selectContainer">
                        <select v-model="mainDelivery" @change="changeShippingMethod">
                            <option :value="delivery.delivery_id"
                                    :data-price="delivery.delivery.price"
                                    :data-time="delivery.delivery.delivery_time"
                                    v-for="delivery in currentCountry.deliveries">
                ...
                <p class="text-right" v-if="mainDelivery == 13">{{ trans.vars.Contacts.contactsLabelAddressShowroom }} {{ trans.vars.Contacts.addressSiteShopMDAdress }}</p>
                ...
                <div class="promoCode">
                    ...
                    <a :href="'/' + $lang + '/' + site + '/obtain'" target="_blank">{{ trans.vars.Promocode.promoHow }}</a>
                </div>
                <div class="" v-if="priceType == 'main'">
                    <div class="bloc total"><span>{{ trans.vars.Orders.orderSubtotal }}:</span> <span>{{ parseFloat(amount).toFixed(2) }} {{ $mainCurrency }}</span></div>
                    <div class="bloc total"><span>{{ trans.vars.Shipping.shipping }}: </span> <span v-if="shippingPrice"> {{ shippingPrice }} {{ $mainCurrency }} </span> <span v-else> 0 {{ $mainCurrency }} </span></div>
                ...
                <div class="" v-if="priceType == 'personal'">
                ...
                <span v-if="mode == 'order-payment'">
                    <label class="checkContainer">
                        ...
                        <a :href="'/' + $lang + '/' + site + '/terms'" target="_blank">
                            {{ trans.vars.PagesNames.pageNameTermsConditions }}
                        </a>
                        {{ trans.vars.FormFields.and }}
                        <a :href="'/' + $lang + '/' + site + '/refund'" target="_blank">
    ...
    export default {
        ...
        mounted() {
            ...
            bus.$on('changeCountry', data => {
                ...
                this.mainDelivery = this.currentCountry.main_delivery.delivery_id;
                console.log(this.currentCountry.deliveries);

                let currentShipping = null;

                this.currentCountry.deliveries.filter(item => {
                    if (item.delivery.id == this.mainDelivery) {
                        currentShipping = item.delivery
                    }
                });

                this.shippingPrice = currentShipping ? currentShipping.price : false;
                // this.originalShippingPrice = this.currentCountry.main_delivery ? this.currentCountry.main_delivery.price : false;
                this.originalShippingPrice = currentShipping ? currentShipping.price : false;
        ...
        methods: {
            ...
            continueToPayment(){
                ...
                if (!this.addShippingInfo) {
                    ...
                    // bus.$emit('ga-event-addShippingInfo', this.currentCountry);
            ...
            loadCountries(){
                axios.post('/' + this.$lang + '/' + this.site +'/get-countries')
                    .then(response => {
                        ...
                        this.mainDelivery = response.data.mainDelivery ? response.data.mainDelivery: response.data.currentCountry.main_delivery.delivery_id;
