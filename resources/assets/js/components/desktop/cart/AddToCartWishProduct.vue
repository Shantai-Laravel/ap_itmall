<template>
    <div>
        ...
        <div class="addToCart" @click="moveToCart(product)">
            <span>
    ...
    export default {
        ...
        methods: {
            ...
            addProductToCartAction(product) {
                axios
                    .post("/" + this.$lang + "/" + this.site + "/add-product-to-cart-from-wish", {
                    ...
                    .then((response) => {
                        ...
                        bus.$emit("updateWishBox", response.data.wish.products);
                        ...
                        this.wishListProducts = response.data.wish.products;
                        this.wishListSets = response.data.wish.sets;
                    })
                    .catch((e) => {
                        console.log(e);
                    });
            ...
            addAnimation() {
                document.getElementById("cart").classList.add("heartBeat");
                setInterval(() => {
                    document.getElementById("cart").classList.remove("heartBeat");
                }, 2000);
            },
...
<style media="screen">
    .sizeContainer div:first-child span:first-child {
        color: #42261d;
    }
    .disabled {
        opacity: 0.5;
