<template>
    ...
    <tr class="row100">
        ...
        <td class="column-button">
            <span><i>{{ strCut(product.translations[0].name) }}</i></span>
            <button @click="addAdjacentProductList()" type="button" class="btn btn-default btn-sm" data-toggle="modal" :data-target="'#adjacent-products-modal' + product.id"><i class="fa fa-point"></i> <small>Adjacent/Similar</small> </button>
        </td>
        <!-- <td class="column-button">
            <span><i>{{ strCut(product.translations[0].name) }}</i></span>
                ...
                <div class="modal-content">
                    <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="col-md-4 image-wrapp" v-for="image in images" v-if="!image.deleted">
                                    <div class="image-btns text-center">
                                        ...
                                        <span :class="[image.main == 1 ? 'active-btn' : '']">
                                            <i class="fa fa-edit" @click="mainImage(image)"></i>
                                        </span>

                                        <span :class="[image.type === 'set' ? 'active-btn' : '']">
                                            <small @click="setImage(image)">set</small>
                                        </span>
                                        ...
                                        <span class="remove-btn"><i class="fa fa-remove" @click="removeImage(image)"></i></span>
                                    </div>
                ...
                <div class="modal-content">
                    <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="col-md-3 image-wrapp" v-for="image in imagesFB" v-if="!image.deleted">
        ...
        <div class="modal fade bd-example-modal-lg settings-modal" :id="'§' + product.id" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                ...
                <div class="modal-content">
                    <div class="row">
                        <div class="col-md-12">
                            <h5 class="text-center"><b>Images : </b> {{ product.translations[0].name }}</h5><hr>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="col-md-4">
        ...
        <!-- Adjacent Products Modal-->
        <div class="modal fade bd-example-modal-lg settings-modal" :id="'adjacent-products-modal' + product.id" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="loading" v-if="loading"><div class="lds-ripple"><div></div><div></div></div></div>
                <div class="modal-content">
                    <div class="row">
                        <div class="col-md-12">
                            <h5 class="text-center">Adjacent/Similars Products : {{ product.translations[0].name }}</h5>
                            <hr>
                        </div>
                        <div class="row">
                            <div class="col-md-12">
                                <div class="col-md-6">
                                    <h5>Adjacent Products: </h5>
                                    <div class="row" v-for="adjacentProduct in adjacentProducts">
                                        <div class="form-group">
                                            <div class="col-md-12">
                                                <input type="checkbox" :checked="product.adjacents.filter(adjacentProd => adjacentProd.adjacent_id === adjacentProduct.id).length" @click="selectAdjacentProduct(adjacentProduct.id)" :id="'category' + adjacentProduct.id + product.alias" class="similar-categories">
                                                <label :for="'category' + adjacentProduct.id + product.alias"><span>{{ adjacentProduct.translation.name }}</span></label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <div class="row" v-for="similarProd in similarCatProducts">
                                        <div class="form-group">
                                            <div class="col-md-12">
                                                <input type="checkbox" :checked="product.similars.filter(similar => similar.similar_id === similarProd.id).length" @click="selectSimilarProduct(similarProd.id)" :id="'category' + similarProd.id + product.alias" class="similar-categories">
                                                <label :for="'category' + similarProd.id + product.alias"><span>{{ similarProd.translation.name }}</span></label>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            selectSimilarProduct(id){
                bus.$emit('startLoading');
                axios.post('/back/auto-upload-select-similar-productList', {productId: this.product.id, similarId: id})
                    .then(response => {
                        bus.$emit('endLoading');
                    })
                    .catch(e => {
                        console.log('error select adjacent product');
                    })
            },
            selectAdjacentProduct(id){
                bus.$emit('startLoading');
                axios.post('/back/auto-upload-select-adjacent-productList', {productId: this.product.id, adjacentId: id})
                    .then(response => {
                        bus.$emit('endLoading');
                    })
                    .catch(e => {
            ...
            addAdjacentProductList(){
                bus.$emit('startLoading');
                axios.post('/back/auto-upload-get-adjacent-productList', {productId: this.product.id})
                    .then(response => {
                        this.adjacentProducts = response.data.adjacents;
                        this.similarCatProducts = response.data.similars;
                        bus.$emit('endLoading');
                    })
                    .catch(e => {
                        console.log('error add adjacent products list');
                    })
            },
            getDillersPrices(){
                bus.$emit('startLoading');
            ...
            removeImage(item){
                if (confirm("Do you really want to delete this image?")) {
                    this.loading = true;
                    axios.post('/back/auto-upload-remove-image', {product_id: this.product.id, id : item.id})
                        .then(response => {
                            item.deleted = 1
                            // this.images = response.data;
                            this.loading = false;
            ...
            setImage(item){
                this.loading = true;
                axios.post('/back/auto-upload-set-image', {product_id: this.product.id, id : item.id})
                    .then(response => {
                        this.product.all_images.forEach(function(entry){
                            if (entry.type == 'set') {
                                entry.type = null
                            }
                        })
            ...
            mainImage(item){
                this.loading = true;
                axios.post('/back/auto-upload-main-image', {product_id: this.product.id, id : item.id})
                    .then(response => {
                        this.product.all_images.forEach(function(entry){
                            if (entry.main == 1) {
                                entry.main = null
